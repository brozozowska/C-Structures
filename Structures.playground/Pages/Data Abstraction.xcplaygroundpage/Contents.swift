...
let songs = [
    Song(title: "Rockstar", artist: "Post Malone", duration: 134, rating: 5),
    Song(title: "Video games", artist: "Lana del Ray", duration: 194, rating: 3),
    Song(title: "Мозги", artist: "Комсомольск", duration: 118, rating: 4)
]

/* Declare the songInformation function here */
func informationFor (song: Song) -> String {
    return "\"\(song.title)\" by \(song.artist), duration \(song.duration) seconds and rating \(song.rating)"
}

/* Write a for...in loop here */
for song in songs {
    print(informationFor(song: song))
}
/*:
